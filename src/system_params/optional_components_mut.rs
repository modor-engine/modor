...
pub(crate) mod internal {
    ...
    impl<'a, C> ComponentIter<'a, C> {
        fn new(components: Option<IterMut<'a, C>>, entity_count: usize) -> Self {
            let component_count = components.as_ref().map_or(0, IterMut::len);
            Self {
...
mod component_mut_option_system_param_tests {
    ...
    fn retrieve_properties() {
        let properties = <Option<&mut u32>>::properties();
    ...
    fn retrieve_stream() {
        ...
        assert_eq!(
            <Option<&mut u32>>::stream_next(&mut stream),
            Some(Some(&mut 20))
        );
    ...
    fn retrieve_query_iter_mut() {
        ...
        let iter = <Option<&mut u32>>::query_iter_mut(&mut guard_borrow, &iter_info);

        assert_iter!(
            iter,
            [Some(&mut 20), None, None, Some(&mut 40), Some(&mut 50)]
        );
    }
    ...
    fn retrieve_reversed_query_iter_mut() {
        ...
        let iter = <Option<&mut u32>>::query_iter_mut(&mut guard_borrow, &iter_info).rev();

        assert_iter!(
            iter,
            [Some(&mut 50), Some(&mut 40), None, None, Some(&mut 20)]
        );
    }
}
