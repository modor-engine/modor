...
///     let gamepads = &ctx.root::<Inputs>().gamepads;
...
///     let gamepads = &mut ctx.root::<Inputs>().gamepads;
///     gamepads.refresh();
///     let gamepad_id = 0;
...
impl Gamepads {
    ...
    ///
    /// This should be called just before updating the gamepads state.
...
impl Gamepad {
    ...
    fn sync_d_pad(&mut self) {
        ...
        if self.has_d_pad_button {
            self[GamepadStick::DPad] = normalization::normalized_direction(
