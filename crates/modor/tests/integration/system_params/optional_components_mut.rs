use crate::system_params::{
    ...
    NoValueFilter, OtherValue, QueryTester, RareComponent, Value, DISABLED_ID, MATCHING1_ID,
...
#[modor_test]
fn run_query_iter() {
    QueryTester::<(Option<&mut Value>, Filter<With<Enabled>>)>::run(|q| {
...
fn run_query_filtered_iter() {
    QueryTester::<(Option<&mut Value>, Filter<With<Enabled>>)>::run(|q| {
        ...
        let values = [Some(VALUE2), Some(VALUE2_CLONE)];
        assert_iter(q.iter().map(|v| v.0.map(|v| v.0)), values);
        let values = [Some(VALUE2_CLONE), Some(VALUE2)];
        assert_iter(q.iter().rev().map(|v| v.0.map(|v| v.0)), values);
    });
}
...
fn run_systems_in_parallel() {
    assert!(!are_systems_run_in_parallel!(Option<&mut Value>, ()));
}
...
impl Tracked {
    #[run]
    fn update(&mut self, param: Option<&mut Value>, _: Filter<With<Enabled>>) {
