...
fn create_glob() {
    ...
    assert_eq!(glob1.get(&app).0, "+a");
    assert_eq!(glob2.get(&app).0, "+b");
    glob1.get_mut(&mut app).0 += "c";
    assert_eq!(glob1.get(&app).0, "+ac");
    assert_eq!(glob2.get(&app).0, "+b");
}
...
fn create_glob_ref() {
    ...
    assert_eq!(glob1_ref.get(&app).0, "+a");
    assert_eq!(glob2_ref.get(&app).0, "+b");
    glob1.get_mut(&mut app).0 += "c";
    assert_eq!(glob1_ref.get(&app).0, "+ac");
    assert_eq!(glob2_ref.get(&app).0, "+b");
}
...
fn access_all_globals() {
    ...
    assert_eq!(globals.get(0).map(|l| l.0.as_str()), Some("+a"));
    assert_eq!(globals.get(1).map(|l| l.0.as_str()), Some("+b"));
    assert_eq!(globals.get(2).map(|l| l.0.as_str()), None);
    assert_eq!(globals.get_mut(0).map(|l| l.0.as_str()), Some("+a"));
    assert_eq!(globals.get_mut(1).map(|l| l.0.as_str()), Some("+b"));
    assert_eq!(globals.get_mut(2).map(|l| l.0.as_str()), None);
    globals[&glob1].0 += "a";
    assert_eq!(globals[&glob1].0, "+aa");
    assert_eq!(globals[&glob2].0, "+b");
    let iterator: Vec<_> = globals.iter().map(|l| l.0.as_str()).collect();
    assert_eq!(iterator, vec!["+aa", "+b"]);
    ...
    let iterator: Vec<_> = globals
        ...
        .collect();
    assert_eq!(iterator, vec![(0, "+aa"), (1, "+b")]);
    let iterator: Vec<_> = globals
        .iter_mut_enumerated()
        .map(|(i, l)| (i, l.0.as_str()))
        .collect();
...
fn access_all_globals_after_value_dropped() {
    ...
    assert_eq!(deleted_items[0].1 .0, "+a");
    assert_eq!(globals.get(0).map(|l| l.0.as_str()), None);
    assert_eq!(globals.get(1).map(|l| l.0.as_str()), Some("+b"));
    assert_eq!(globals.get(2).map(|l| l.0.as_str()), None);
    let iterator: Vec<_> = globals.iter().map(|l| l.0.as_str()).collect();
    assert_eq!(iterator, vec!["+b"]);
    let iterator: Vec<_> = globals
        .iter_enumerated()
        .map(|(i, l)| (i, l.0.as_str()))
        .collect();
    assert_eq!(iterator, vec![(1, "+b")]);
}
