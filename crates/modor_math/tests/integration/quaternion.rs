...
fn create_from_axis_angle() {
    ...
    let quat = Quat::from_axis_angle(Vec3::xyz(1., 2., 3.), PI.mul_add(6., FRAC_PI_2));
    ...
    let quat = Quat::from_axis_angle(Vec3::xyz(-1., -2., -3.), FRAC_PI_2 - 6. * PI);
    let axis = quat.axis().unwrap();
...
fn create_with_rotation() {
    ...
    assert_abs_diff_eq!(axis.y, 0.);
    assert_abs_diff_eq!(axis.z, 0.);
    let quat = Quat::from_axis_angle(Vec3::xyz(0., 1., 0.), FRAC_PI_2);
    let new_quat = quat.with_rotation(Quat::from_axis_angle(Vec3::xyz(0., 1., 0.), FRAC_PI_4));
    let axis = new_quat.axis().unwrap();
    assert_abs_diff_eq!(new_quat.angle(), 3. * FRAC_PI_4);
    assert_abs_diff_eq!(axis.x, 0.);
    assert_abs_diff_eq!(axis.y, 1.);
    assert_abs_diff_eq!(axis.z, 0.);
    let quat = Quat::from_axis_angle(Vec3::xyz(0., 0., 1.), FRAC_PI_2);
    let new_quat = quat.with_rotation(Quat::from_axis_angle(Vec3::xyz(0., 0., 1.), FRAC_PI_4));
    let axis = new_quat.axis().unwrap();
    assert_abs_diff_eq!(new_quat.angle(), 3. * FRAC_PI_4);
    assert_abs_diff_eq!(axis.x, 0.);
    assert_abs_diff_eq!(axis.y, 0.);
