...
pub(crate) fn run_with_frame_rate<F>(start: Instant, frame_rate: FrameRateLimit, f: F)
where
    F: FnOnce(),
{
    f();
    if let FrameRateLimit::Fps(frames_per_second) = frame_rate {
...
mod utils_tests {
    ...
    fn run_with_frame_rate() {
        modor_internal::retry!(
            10,
            assert_duration(FrameRateLimit::Unlimited, 100, 100, 150)
        );
        modor_internal::retry!(10, assert_duration(FrameRateLimit::VSync, 100, 100, 150));
        modor_internal::retry!(10, assert_duration(FrameRateLimit::Fps(0), 100, 100, 150));
        modor_internal::retry!(10, assert_duration(FrameRateLimit::Fps(1), 500, 1000, 1200));
        modor_internal::retry!(10, assert_duration(FrameRateLimit::Fps(5), 100, 200, 300));
    }

    fn assert_duration(
        frame_rate: FrameRateLimit,
