...
pub(super) fn convert_mouse_button(button: event::MouseButton) -> MouseButton {
    match button {
...
pub(super) fn convert_keyboard_key(code: VirtualKeyCode) -> Key {
    match code {
...
pub(super) fn convert_gamepad_button(button: gilrs::Button) -> Option<GamepadButton> {
    match button {
...
pub(super) fn convert_gamepad_axis(button: gilrs::Axis) -> Option<GamepadAxis> {
    match button {
...
pub(in crate::runner) struct Gamepads {
...
impl Gamepads {
    pub(super) fn new() -> Self {
        Self {
            #[cfg(not(target_os = "android"))]
            gilrs: gilrs::Gilrs::new().expect("cannot retrieve gamepad information"),
        }
    }

    pub(super) fn plugged_gamepads_ids(&self) -> impl Iterator<Item = u64> + '_ {
        #[cfg(not(target_os = "android"))]
        {
            self.gilrs
                .gamepads()
    ...
    pub(super) fn next_event(&mut self) -> Option<gilrs::ev::Event> {
        #[cfg(not(target_os = "android"))]
        {
